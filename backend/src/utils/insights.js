...
async function buildTimeline(prisma, patientId, daysOrOpts = 7) {
  ...
  const items = [
    ...exLogs.map((l) => ({ id: l.id, patientId, kind: 'exercise', source: l.aiAssisted ? 'AI-assisted exercise' : src(l.loggedBy?.role), sourceRecordId: l.id, at: l.loggedAt, title: `${l.assignment?.exercise?.name || 'Exercise'} — ${l.status}${l.aiAssisted ? ' · AI-assisted' : ''}`, detail: `${l.aiAssisted && l.detectedReps != null ? `${l.detectedReps} detected reps` : `Reps ${l.repsDone ?? '–'}`} · ${l.durationMin ?? '–'} min${l.remarks ? ` · ${l.remarks}` : ''}`, level: l.status === 'missed' ? 'amber' : 'green' })),
